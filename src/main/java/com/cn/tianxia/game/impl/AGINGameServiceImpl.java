 ...
 * 功能概要：AGService实现类
 */
public class AGINGameServiceImpl implements GameReflectService {
    ...
    public JSONObject transferIn(GameTransferVO gameTransferVO) {
        ...
        try {
            // 调用预转账
            String msg = PrepareTransferCredit(ag_username, billno, "IN", credit + "", ag_password, "CNY");
            if (!"0".equals(msg)) {
            ...
            do {
                // 调用确认转账
                msg = TransferCreditConfirm(ag_username, billno, "IN", credit + "", "1", ag_password, "CNY");
                if (!"0".equals(msg)) {
                    int counts = 0;
                    do {
                        Thread.sleep(3000);
                        ...
                        if ("0".equals(msg)) {
                            // 转账订单处理成功
                            return GameResponse
                                    .success("用户【" + username + "】操作天下平台向游戏平台【" + type + "】转入金额业务(游戏上分)业务成功");
                            ...
                            if (counts > 2) {
                                ...
                                // 异常订单,未收到任何回馈的
                                return GameResponse
                                        .process("用户【" + username + "】操作天下平台向游戏平台【" + type + "】转入金额业务(游戏上分)业务异常,需人工审核");
                            }
                        }
                    } while (bflag);
                }
    ...
    public JSONObject forwardGame(GameForwardVO gameForwardVO) {
        ...
        try {
            if ("mobile".equals(model)) {
                String url = forwardMobileGame(ag_username, ag_password, ip, sid, gameID, handicap);
                return GameResponse.form(url);
            } else {
                String url = forwardGame(ag_username, ag_password, ip, sid, gameID, handicap);
                return GameResponse.form(url);
            }
        } catch (Exception e) {
            e.printStackTrace();
