 ...
 *  
 */ 
public class AGINFunServiceImpl{ 
	...
	private final static Logger logger = LoggerFactory.getLogger(AGINFunServiceImpl.class);
	
	public AGINFunServiceImpl(Map<String, String> pmap) {
	...
	public String CheckOrCreateGameAccout(String loginname,  String password,
		...
		logger.info("AGINFun【检测并创建游戏账号】请求参数==========>"+tagUrl);
		...
		logger.info("AGINFun【检测并创建游戏账号】响应参数<=========="+info);
	...
	public String GetBalance(String loginname,  String password,String cur) { 
		...
		String tagUrl=getAGUrl(api_url,xmlString);
		logger.info("AGINFun【查询余额】请求参数==========>"+tagUrl);
		xmlString=sendPost(api_cagent, tagUrl);
		String info="";
		String msg="";
		try {
        ...
        logger.info("AGINFun【查询余额】响应参数<=========="+info);
		if("error".equals(info)){
			FileLog f=new FileLog(); 
			Map<String,String> map =new HashMap<>();
			map.put("tagUrl", "tagUrl"); 
	...
	public String PrepareTransferCredit(String loginname, String billno, String type,
		...
		String tagUrl=getAGUrl(api_url,xmlString);
        logger.info("AGINFun【预备转账】请求参数==========>"+tagUrl);
		xmlString=sendPost(api_cagent, tagUrl);
		String info="";
		String msg="";
		try {
        ...
        logger.info("AGINFun【预备转账】响应参数<=========="+info);
		if(!"0".equals(info)){
			FileLog f=new FileLog(); 
			Map<String,String> map =new HashMap<>();
			map.put("tagUrl", "tagUrl"); 
	...
	public String TransferCreditConfirm(String loginname, String billno, String type,
		...
		String tagUrl=getAGUrl(api_url,xmlString);
        logger.info("AGINFun【确认转账】请求参数==========>"+tagUrl);
		xmlString=sendPost(api_cagent, tagUrl);
		String info="";
		String msg="";
		try {
        ...
        logger.info("AGINFun【确认转账】响应参数<=========="+info);
		if(!"0".equals(info)){
			FileLog f=new FileLog(); 
			Map<String,String> map =new HashMap<>();
			map.put("tagUrl", "tagUrl"); 
	...
	public String QueryOrderStatus(String billno,  String cur) {
		...
		String tagUrl=getAGUrl(api_url,xmlString);
        logger.info("AGINFun【检查订单状态】请求参数==========>"+tagUrl);
		xmlString=sendPost(api_cagent, tagUrl);
		String info="";
		String msg="";
		try {
        ...
        logger.info("AGINFun【检查订单状态】响应参数<=========="+info);
		if("error".equals(info)){
			FileLog f=new FileLog(); 
			Map<String,String> map =new HashMap<>(); 
			map.put("actype", actype);
	...
	public String forwardGame(String loginname, String password, String dm, String sid, String gameType) { 
        ...
        logger.info("AGINFun【获取游戏跳转连接】请求参数==========>"+xmlString);
		xmlString=getAGUrl(api_url_game, xmlString);
        logger.info("AGINFun【获取游戏跳转连接】响应参数<=========="+xmlString);
		return xmlString;
	}
