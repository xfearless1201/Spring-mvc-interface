 ...
 * 
 */
public class JFKPayServiceImpl implements PayService {
	private final static Logger logger = LoggerFactory.getLogger(JFKPayServiceImpl.class);
	...
	public JFKPayServiceImpl(Map<String, String> pmap) {
	...
	public JSONObject scanPay(Map<String, String> payParamsMap) {
		...
		try {
			...
			logger.info("金付卡扫码" + payType + "请求参数:" + synParam.toString());
			...
			try {
				...
				logger.info("金付卡扫码" + payType + "服务器返回:" + respTxt);
				if (result.getStatus() != 200) {
					// 请求服务器失败
					logger.info("金付卡扫码http请求失败:" + result.getStatus());
				...
				if (respTxt.contains("</html>")) {
					// 服务器返回异常
					logger.info("金付卡扫码服务器返回异常:" + result.getStatus());
					WriteInFile(payType, result.getStatus() + "", respTxt);
					return r_json;
				}
			} catch (Exception e) {
				...
				e.printStackTrace();
				logger.info("金付卡扫码异常:" + result.getStatus());
			...
			if (respTxt == null || "".equals(respTxt)) {
				logger.info("金付卡扫码服务器返回数据为空:" + result.getStatus());
				WriteInFile(payType, result.getStatus() + "", respTxt);
				return r_json;
			}
			...
			if (respCode != null && !"".equals(respCode)) {
				if (!"1000".equals(respCode)) {// 返回1000表示成功。当dq_code为1000时，订单状态才有效。
					logger.info("金付卡扫码订单无效respCode：" + respCode);
					WriteInFile(payType, result.getStatus() + "", respTxt);
					return r_json;
				}
			}
	...
	public static void main(String[] args) {
		...
		JFKPayServiceImpl.testCallbeak();
		...
		//// logger.info("金付卡json配置:" + json.toString());
