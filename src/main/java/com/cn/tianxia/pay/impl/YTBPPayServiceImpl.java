 ...
 * @Description 通宝支付
 * @author Hardy
 * @Date 2019年1月24日 下午6:17:17
 * @version 1.0.0
 */
public class YTBPPayServiceImpl implements PayService {

    private final static Logger logger = LoggerFactory.getLogger(YTBPPayServiceImpl.class);
    private String memberid;
    private String payUrl;
    private String notifyUrl;
    private String md5Key;

    public YTBPPayServiceImpl(Map<String,String> map) {
    ...
    public JSONObject smPay(PayEntity payEntity) {
        logger.info("[TBZF]通宝支付扫码支付开始================START============");
        try {
            //获取支付请求参数
            ...
            logger.info("[TBZF]通宝支付扫码支付请求参数报文:{}",JSONObject.fromObject(data).toString());
            //生成form请求表单
            String formStr = HttpUtils.generatorForm(data, payUrl);
            logger.info("[TBZF]通宝支付扫码支付生成form请求表单结果:{}",formStr);
            return PayResponse.sm_form(payEntity, formStr, "扫码支付下单成功");
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[TBZF]通宝支付扫码支付异常:{}",e.getMessage());
            return PayResponse.error("[TBZF]通宝支付扫码支付异常");
        }
    }

    @Override
    public String callback(Map<String, String> data) {
        logger.info("[YTBZF]通宝支付回调验签开始============START==============");
        try {
            
            String sourceSign = data.get("sign");
            logger.info("[YTBZF]通宝支付回调验签获取原签名串:{}",sourceSign);
            
            String sign = generatorSign(data);
            logger.info("[YTBZF]通宝支付回调验签生成签名串:{}",sign);
            
            if(sourceSign.equalsIgnoreCase(sign)) return "success";
            
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[YTBZF]通宝支付回调验签异常:{}",e.getMessage());
        }
        return "faild";
    }
    ...
    private Map<String,String> sealRequest(PayEntity entity) throws Exception{
        logger.info("[TBZF]通宝支付组装支付请求参数开始==============START==============");
        try {
...
//            data.put("pay_producturl","");//商户链接地址否否
            ...
            e.printStackTrace();
            logger.info("[TBZF]通宝支付组装支付请求参数异常:{}",e.getMessage());
            throw new Exception("[TBZF]通宝支付组装支付请求参数异常");
        }
    }
    ...
    private String generatorSign(Map<String,String> map) throws Exception{
        logger.info("[TBZF]通宝支付生成签名开始============START=============");
        try {
            ...
            while(iterator.hasNext()){
                ...
                if(StringUtils.isBlank(val) || "sign".equalsIgnoreCase(key) 
                        || "pay_productname".equalsIgnoreCase(key)) continue;
                
                sb.append(key).append("=").append(val).append("&");
            }
            
            String signStr = sb.append("key=").append(md5Key).toString();
            logger.info("[TBZF]通宝支付生成待签名串:{}",signStr);

            String sign = MD5Utils.md5toUpCase_32Bit(signStr);//大写
            logger.info("[TBZF]通宝支付生成签名串:{}",sign);
            
            return sign;
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[TBZF]通宝支付生成签名异常:{}",e.getMessage());
            throw new Exception("[TBZF]通宝支付生成签名异常");
        }
    }
}
