 ...
 * 
 */
public class ZTBPayServiceImpl implements PayService {
	private String merchant_code;// 商 家 号
	private String service_type;// 服务类型
	private String interface_version;// 扫描支付接口版本
	private String b2c_interface_version; // 网银支付接口版本
	private String input_charset;// 字符编码
	private String notify_url;// 服务器异步通知地址
	private String sign_type;
	private String payurl;// 支付地址
	private String merchant_private_key;// 商户私钥
	private String dinpay_public_key;// 商户公钥
	private String h5_version;// h5接口版本
	private String h5_apiUrl;
	...
	private String redo_flag;// 订单是否允许重复提交//0是,1否,""空
	private String product_code;// 商品编号
	private String product_num; // 商品数量
	private String product_desc; // 商品描述
	private String pay_type;// 支付类型取值如下（必须小写，多选时请用逗号隔开）b2c,plateform,dcard,express,weixin,alipay
	private String extend_param;// 业务扩展参数
	private String extra_return_param;// 回传参数
	private String show_url;// 商品展示
	private String scan_notify_url; // 扫描回调接口

	private final static Logger logger = LoggerFactory.getLogger(ZTBPayServiceImpl.class);

	public ZTBPayServiceImpl(Map<String, String> pmap) {
	...
	public String PayBank(String order_amount, String bank_code, String order_no, Map<String, String> reqMap) {
		...
		if ("RSA-S".equals(sign_type)) {// sign_type = "RSA-S"
			 ...
			 * 获取商户公钥（merchant_public_key）并且将商户公钥上传到智通宝商家后台"公钥管理"（如何获取和上传请看《
	...
	public String ScanPay(Map<String, String> scanMap) {
		...
		if ("RSA-S".equals(sign_type)) { 
			try {
				...
				result = doPost(reqUrl, reqMap, "utf-8"); 
			} catch (Exception e) {
				logger.info("智通宝扫码签名异常！");
				e.printStackTrace();
			}
		}
	...
	public String callback(HttpServletRequest request) {
		...
		logger.info("智通宝返回的签名参数排序：" + signInfo.length() + " -->" + signInfo);
		logger.info("智通宝返回的签名：" + dinpaySign.length() + " -->" + dinpaySign);
		boolean result = false;

		if ("RSA-S".equals(sign_type)) { // sign_type = "RSA-S"

			/**
			 * 1)dinpay_public_key，智通宝公钥，每个商家对应一个固定的智通宝公钥（
			 * 不是使用工具生成的商户公钥merchant_public_key，不要混淆），
			 * 即为智通宝商家后台"支付管理"->"公钥管理"->"智通宝公钥"里的绿色字符串内容
			 * 2)demo提供的dinpay_public_key是测试商户号1111110166的智通宝公钥，
			 * 请自行复制对应商户号的智通宝公钥进行调整和替换
			 */
			...
			try {
				...
				e.printStackTrace();
				logger.info("签名异常！");
			} // 验签 signInfo智通宝返回的签名参数排序， dinpay_public_key智通宝公钥，
				// dinpaySign智通宝返回的签名
		}
		...
		// dinpaySign); // 验签 merchant_code为商户号， signInfo智通宝返回的签名参数排序，
		// dinpaySign智通宝返回的签名
		// }
		//
		...
		if (result) {
			logger.info("智通宝扫码签名成功！");
			return "SUCCESS";
		} else {
			logger.info("智通宝扫码签名失败！");
