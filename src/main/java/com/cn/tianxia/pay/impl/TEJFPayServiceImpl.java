...
/**
 * 
 * @ClassName TEJFPayServiceImpl
 * @Description 特捷付支付接口
 * @author Hardy
 * @Date 2018年12月18日 上午11:07:04
 * @version 1.0.0
 */
public class TEJFPayServiceImpl implements PayService {

    //日志
    private static final Logger logger = LoggerFactory.getLogger(TEJFPayServiceImpl.class);
    ...
    //构造器,初始化参数
    public TEJFPayServiceImpl(Map<String,String> data) {
    ...
    public JSONObject wyPay(PayEntity payEntity) {
        logger.info("[TEJF]特捷付支付网银支付开始====================START======================");
        try {
            //获取支付请求参数
            ...
            data.put("sign", sign);
            logger.info("[TEJF]特捷付支付网银支付请求参数报文:{}",JSONObject.fromObject(data).toString());
            String formStr = HttpUtils.generatorForm(data, wyPayUrl);
            logger.info("[TEJF]特捷付支付网银支付生成form表单结果:{}",formStr);
            return PayResponse.wy_form(payEntity.getPayUrl(),formStr);
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[TEJF]特捷付支付网银支付异常:{}",e.getMessage());
            return PayResponse.error("[TEJF]特捷付支付网银支付异常");
        }
    }

    @Override
    public JSONObject smPay(PayEntity payEntity) {
        logger.info("[TEJF]特捷付支付扫码支付开始================START=================");
        try {
            //获取支付请求参数
            ...
            String sign = generatorSign(data);
            data.put("sign", sign);
            logger.info("[TEJF]特捷付支付扫码支付请求参数报文:{}",JSONObject.fromObject(data).toString());
            int type = checkPayChannel(payEntity.getPayCode());
            if(type == 1){
                //H5直连
                String formStr = HttpUtils.generatorForm(data, wyPayUrl);
                logger.info("[TEJF]特捷付支付扫码支付生成form表单结果:{}",formStr);
                return PayResponse.sm_form(payEntity, formStr, "下单成功");
                
            }
            String response = HttpUtils.toPostForm(data, smPayUrl);
            if(StringUtils.isBlank(response)){
                logger.info("[TEJF]特捷付支付扫码支付发起HTTP请求无响应结果");
                return PayResponse.error("[TEJF]特捷付支付扫码支付发起HTTP请求无响应结果");
            }
            logger.info("[TEJF]特捷付支付扫码支付发起HTTP请求响应结果:{}",response);
            //解析响应结果
            ...
            return PayResponse.error("[TEJF]特捷付支付扫码支付失败:"+response);
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[TEJF]特捷付支付扫码支付异常:{}",e.getMessage());
            return PayResponse.error("[TEJF]特捷付支付扫码支付异常");
        }
    }

    @Override
    public String callback(Map<String, String> data) {
        logger.info("[TEJF]特捷付支付回调验签开始==================START===============");
        try {
            
            //获取原签名串
            String sourceSign = data.get("sign");
            logger.info("[TEJF]特捷付支付回调验签获取原签名串:{}",sourceSign);
            String sign = generatorSign(data);
            logger.info("[TEJF]特捷付支付回调验签生成签名串:{}",sign);
            if(sourceSign.equalsIgnoreCase(sign)) return "success";
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[TEJF]特捷付支付回调验签异常:{}",e.getMessage());
        }
        return "faild";
    }
    ...
    private Map<String,String> sealRequest(PayEntity entity,int type) throws Exception{
        logger.info("[TEJF]特捷付支付组装支付请求参数开始=====================START==================");
        try {
...
//            data.put("trade_msg","");//交易请求信息   根据支付类型不同，约定不同的传值
            ...
            e.printStackTrace();
            logger.info("[TEJF]特捷付支付组装支付请求参数异常:{}",e.getMessage());
            throw new Exception("[TEJF]特捷付支付组装支付请求参数异常");
        }
    }
    ...
    private String generatorSign(Map<String,String> data) throws Exception{
        logger.info("[TEJF]特捷付支付生成签名串开始===================START==================");
        try {
            ...
            logger.info("[TEJF]特捷付支付生成待签名串:{}",signStr);
            String sign = MD5Utils.md5toUpCase_32Bit(signStr).toLowerCase();
            logger.info("[TEJF]特捷付支付生成加密签名串:{}",sign);
            return sign;
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[TEJF]特捷付支付生成签名串异常:{}",e.getMessage());
            throw new Exception("[TEJF]特捷付支付生成签名串异常");
        }
    }
